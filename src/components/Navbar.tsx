...
import { Menu, X, ChevronDown, Layers, BookOpen, Globe, TrendingUp, BarChart3, Instagram } from 'lucide-react';
...
function Navbar() {
  ...
  const getCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
      ...
      'Dijital Pazarlama': 'bg-purple-500/30 text-purple-200 border-purple-400/50',
  ...
  return (
    <nav className="fixed w-full z-50 flex justify-center pt-6 px-4">
      <div className={`relative w-full max-w-5xl transition-all duration-300 rounded-2xl border ${isScrolled ? 'bg-zinc-900 border-zinc-800/50 shadow-2xl shadow-primary/10' : 'bg-zinc-900 border-zinc-800/30 shadow-xl shadow-primary/5'}`}>
        <div className="flex justify-between items-center h-16 px-6">
          <button onClick={() => navigate('/')} className="flex items-center gap-2">
            <Layers className="w-8 h-8 text-accent-light" />
            <span className="text-2xl font-bold text-white">katman<span className="text-accent-light">labs</span></span>
          </button>
          ...
          <div className="hidden md:flex items-center gap-6">
                  ...
                  <div className="bg-zinc-950/95 backdrop-blur-xl border border-zinc-700/80 rounded-2xl shadow-2xl p-6">
                    ...
                    {services.map((service, index) => (
                      ...
                      >
                        ...
                        <div className={`mb-2 transition-transform duration-700 ${
                          service.featured ? 'text-pink-400 group-hover:rotate-360' : 'text-accent-light group-hover:rotate-360'
                        }`}>
                          {service.icon}
                        </div>
                        <h4 className={`text-sm font-semibold text-center transition-colors ${
                          service.featured ? 'text-pink-100 group-hover:text-pink-300' : 'text-white group-hover:text-accent-light'
                        }`}>
                          {service.title}
                        </h4>
                      </Link>
            ...
            <button onClick={() => scrollToSection('process')} className="nav-link-hover px-3 py-2 text-gray-300 text-sm font-medium">Süreç</button>
                          ...
                          {rowBlogs.map((blog) => (
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <span className={`inline-block text-xs px-2.5 py-1 rounded-full border font-medium ${getCategoryColor(blog.category)}`}>
                                  {blog.category}
                                </span>
                              </div>
                            </Link>
            ...
            <button onClick={() => scrollToSection('pricing')} className="nav-link-hover px-3 py-2 text-gray-300 text-sm font-medium">Fiyatlar</button>
            <button onClick={() => scrollToSection('contact')} className="px-6 py-2 bg-gradient-to-r from-accent-light to-accent text-white rounded-lg hover:shadow-lg hover:shadow-accent/50 hover:scale-105 transition-all duration-300 text-sm font-semibold">İletişim</button>
          </div>
          ...
          <div className="md:hidden bg-zinc-900/95 border-t border-zinc-800/50 rounded-b-2xl">
            <div className="px-4 py-4 space-y-3">
              <div className="space-y-2">
                <div className="px-4 py-2 text-gray-400 text-sm font-semibold">Hizmetler</div>
                {services.map((service, index) => (
                  <Link
                    key={index}
                    to={`/hizmet/${service.slug}`}
              ...
              <div className="border-t border-zinc-800/50 pt-3 space-y-3">
                <button onClick={() => scrollToSection('process')} className="block w-full text-left px-4 py-2 text-white hover:bg-gradient-to-r hover:from-accent-light/20 hover:to-accent/20 rounded-lg transition-all duration-300">Süreç</button>
                <button onClick={() => scrollToSection('blog-gallery')} className="block w-full text-left px-4 py-2 text-white hover:bg-gradient-to-r hover:from-accent-light/20 hover:to-accent/20 rounded-lg transition-all duration-300">Blog</button>
                <button onClick={() => scrollToSection('pricing')} className="block w-full text-left px-4 py-2 text-white hover:bg-gradient-to-r hover:from-accent-light/20 hover:to-accent/20 rounded-lg transition-all duration-300">Fiyatlar</button>
                <button onClick={() => scrollToSection('contact')} className="block w-full px-4 py-2 bg-gradient-to-r from-accent-light to-accent text-white rounded-lg font-semibold hover:shadow-lg hover:shadow-accent/50 transition-all duration-300">İletişim</button>
              </div>
            </div>
          </div>
        )}
